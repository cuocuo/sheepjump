...
public class Shengcun extends Layer implements IContactListener 
{
	...
	Shengcun()
	{
		...
		//背景
		Texture2D bc=Texture2D.makePNG(R.drawable.scbc);
		...
		//移动的乌云
		wuyun = new WuyunTwo[6];
		Random b =new Random();
		for (int i3= 0; i3 < wuyun.length; i3++)
		{
			wuyun[i3]=new WuyunTwo(mWorld);
			wuyun[i3].mwuyun.getBody().setTransform(b.nextInt((int)s.width/32-1)-s.width/2/32+1.3f,b.nextInt(100), 0);
		}
		...
		//主角
		{
			...
			robotBody.setLinearVelocity(WYPoint.make(0,48));
		...
		//台阶
		Random a=new Random();
		taijie=new TaijieTwo[10];
		{
			for (int i = 0; i <taijie.length; i++)
			{
				
				taijie[i] =new TaijieTwo(mWorld);
			}
			taijie[0].mtaijie.getBody().setTransform(1.4f-s.width/2/32
					,3f, 0);
			taijie[1].mtaijie.getBody().setTransform(taijie[0].mtaijie.getBody().getPosition().x+(a.nextInt(13))
					,taijie[0].mtaijie.getBody().getPosition().y+(2+a.nextInt(3)), 0);
			taijie[2].mtaijie.getBody().setTransform(taijie[0].mtaijie.getBody().getPosition().x+(a.nextInt(13))
					,taijie[1].mtaijie.getBody().getPosition().y+(2+a.nextInt(3)), 0);
			taijie[3].mtaijie.getBody().setTransform(taijie[0].mtaijie.getBody().getPosition().x+(a.nextInt(13))
					,taijie[2].mtaijie.getBody().getPosition().y+(2+a.nextInt(3)), 0);
			taijie[4].mtaijie.getBody().setTransform(taijie[0].mtaijie.getBody().getPosition().x+(a.nextInt(13))
					,taijie[3].mtaijie.getBody().getPosition().y+(2+a.nextInt(3)), 0);
			taijie[5].mtaijie.getBody().setTransform(taijie[0].mtaijie.getBody().getPosition().x+(a.nextInt(13))
					,taijie[4].mtaijie.getBody().getPosition().y+(2+a.nextInt(3)), 0);
			taijie[6].mtaijie.getBody().setTransform(taijie[0].mtaijie.getBody().getPosition().x+(a.nextInt(13))
					,taijie[5].mtaijie.getBody().getPosition().y+(2+a.nextInt(3)), 0);
			taijie[7].mtaijie.getBody().setTransform(taijie[0].mtaijie.getBody().getPosition().x+(a.nextInt(13))
					,taijie[6].mtaijie.getBody().getPosition().y+(2+a.nextInt(3)), 0);
			taijie[8].mtaijie.getBody().setTransform(taijie[0].mtaijie.getBody().getPosition().x+(a.nextInt(13))
					,taijie[7].mtaijie.getBody().getPosition().y+(2+a.nextInt(3)), 0);
			taijie[9].mtaijie.getBody().setTransform(taijie[0].mtaijie.getBody().getPosition().x+(a.nextInt(13))
					,taijie[8].mtaijie.getBody().getPosition().y+(2+a.nextInt(3)), 0);
		}
		//分数
		...
		//水
		{
			Texture2D shuibc=Texture2D.makePNG(R.drawable.shui);
			shuibc.setAntiAlias(false);
			shui=Sprite.make(shuibc);
			bc.autoRelease();
			shui.autoRelease();	
			shui.setPosition(0,-950);
			shui.setContentSize(s.width, s.height);
			shui.setVelocity(0, 280);
			shui.setAutoFit(true);
			mBox2d.addChild(shui);
		}


		addChild(mBox2d);
		mWorld.setGravity(0,-75);//重力
		mWorld.setContactListener(this);//碰撞监听
		setAccelerometerEnabled(true);//加速度
		schedule(new TargetSelector(this, "update(float)",new Object[]{0f}));
		// 每帧调用一次updateLabel2，来更新第二个label
	...
	public void update(float delta)
	{
		...
		if (shui.getPositionY()>-boxy+s.height/2-10)
		{
			
			shui.setVelocity(0, 0);
			mrobot.setSensor(true);
			scores=fenshu;
			over();
		}
		...
		for (int i = 0; i < taijie.length; i++)
		{
			...
			if (taijie[i].mtaijie.getBody().getPosition().y<-boxy/32&&fenshu<=20000)
			{
				...
				taijie[6].mtaijie.getBody().setLinearVelocity(WYPoint.make(-2.2f, 0));
			}
			if (taijie[i].mtaijie.getBody().getPosition().y<-boxy/32&&fenshu>20000&&fenshu<=50000)
			{
				taijie[i].mtaijie.getBody().setTransform(c.nextInt((int)s.width/32-1)-s.width/2/32+1.3f,
						max+(3+c.nextInt(5)), 0);
				taijie[4].mtaijie.getBody().setLinearVelocity(WYPoint.make((float) (3+fenshu/50000), 0));
			...
			if (taijie[i].mtaijie.getBody().getPosition().y<-boxy/32&&fenshu>100000&&fenshu<=200000)
			{
				taijie[i].mtaijie.getBody().setTransform(c.nextInt((int)s.width/32-1)-s.width/2/32+1.3f,
						max+(3+c.nextInt(7)), 0);
			...
			if (taijie[i].mtaijie.getBody().getPosition().y<-boxy/32&&fenshu>200000)
			{
				taijie[i].mtaijie.getBody().setTransform(c.nextInt((int)s.width/32-1)-s.width/2/32+1.3f,
						max+(3+c.nextInt(7)), 0);
		...
		{
			mcao.getBody().setTransform((f.nextInt(9)+3)-s.width/2/32, mcao.getBody().getPosition().y*(f.nextInt(2)+8)/7+s.height/32, 0);
		}
		if (mxuehua.getBody().getPosition().y*32<-boxy)
		{
			mxuehua.getBody().setTransform((f.nextInt(9)+3)-s.width/2/32, mxuehua.getBody().getPosition().y*(f.nextInt(3)+8)/10+s.height/32, 0);
		}
		if (mxiaoniao.getBody().getPosition().y*32<-boxy)
		{
			mxiaoniao.getBody().setTransform((f.nextInt(9)+3)-s.width/2/32, mxiaoniao.getBody().getPosition().y*(f.nextInt(2)+7)/7+s.height/32, 0);
		}
		if (mshandian.getBody().getPosition().y*32<-boxy)
		{
			mshandian.getBody().setTransform((f.nextInt(9)+3)-s.width/2/32, mshandian.getBody().getPosition().y*(f.nextInt(2)+7)/6+s.height/32, 0);
		}
		for (int j = 0; j < wuyun.length; j++)
		{
			float max=wuyun[0].mwuyun.getBody().getPosition().y;
			float chengdu=wuyun.length;
			for (int j1 = 1; j1 < chengdu; j1++)
			{
				
				if (wuyun[j1].mwuyun.getBody().getPosition().y>max)
				{
					max=wuyun[j1].mwuyun.getBody().getPosition().y;
				}
			}
			if (wuyun[j].mwuyun.getBody().getPosition().y<-boxy/32&&fenshu<10000)
			{
				wuyun[j].mwuyun.getBody().setTransform(c.nextInt((int)s.width/32-1)-s.width/2/32+1.3f,
						max+(3+c.nextInt(60)), 0);
			}
			if (wuyun[j].mwuyun.getBody().getPosition().y<-boxy/32&&fenshu>10000)
			{
				wuyun[j].mwuyun.getBody().setTransform(c.nextInt((int)s.width/32-1)-s.width/2/32+1.3f,
						max+(3+c.nextInt(50)), 0);
	...
	public void over()
	{
		...
		running = true;
				Director.getInstance().runOnGLThread(new Runnable() {
					@Override
					public void run() {
						Skeleton.skeleton.play4();
						OverTwo overTwo	=new OverTwo();
						Scene mScene=Scene.make();
						mScene.addChild(overTwo);
						mScene.autoRelease();
	...
	public void preSolve(int contactPointer, int oldManifoldPointer)
	{
		...
		if (fixtureB.equals(mrobot))
		{
			...
			else if (fixtureA.equals(mxiaoniao))
			{
				...
				mrobot.getBody().setLinearVelocity(WYPoint.make(sudu.x,40f));
				scheduleOnce(new TargetSelector(this, "xiaoniao1(float)",new Object[]{0f}));
				scheduleOnce(new TargetSelector(this, "xiaoniao(float)",new Object[]{0f}),2.5f);
				pengzhuang=0;
			}
			else if (fixtureA.equals(mshandian))
			{
				...
				scheduleOnce(new TargetSelector(this, "shandian(float)",new Object[]{0f}),2.5f);
				pengzhuang=0;
			}
		...
		if (pengzhuang==1)
		{
			if ((mrobot.getBody().getLinearVelocity().y<0&&
					mrobot.getBody().getPosition().y>taijie[i].mtaijie.getBody().getPosition().y+1.2f))
			{
	...
	public void cao1(float delta)
	{
		WYSize s= Director.getInstance().getWindowSize();
		Random f= new Random();
		mcao.getBody().setTransform((f.nextInt(9)+3)-s.width/2/32, mcao.getBody().getPosition().y*(f.nextInt(2)+7)/6+s.height/32,0);
	}
	...
	public void xuehua1(float delta)
	{
		WYSize s= Director.getInstance().getWindowSize();
		Random f= new Random();
		mxuehua.getBody().setTransform((f.nextInt(9)+3)-s.width/2/32, mxuehua.getBody().getPosition().y*(f.nextInt(2)+7)/6+s.height/32,0);
	}
	public void xiaoniao(float delta)
	{
		
		mWorld.setGravity(0, -75);
	...
	public void xiaoniao1(float delta)
	{
		WYSize s= Director.getInstance().getWindowSize();
		Random f= new Random();
		mxiaoniao.getBody().setTransform((f.nextInt(9)+3)-s.width/2/32, mxiaoniao.getBody().getPosition().y*(f.nextInt(2)+8)/6+s.height/32,0);
	}
