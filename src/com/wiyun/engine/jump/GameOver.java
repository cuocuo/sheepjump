...
public class GameOver extends Layer
{
	...
	GameOver()
	{
		...
		label1.setText(String.format("your score: %d",Box2d.scores));
		label1.setColor(WYColor3B.make(0, 0, 0));
		label1.setPosition(s.width/2, s.height-DP(210));
		label1.autoRelease();
		addChild(label1);

		BitmapFont font2 = BitmapFont.loadFont(
				R.raw.one);
		if (Integer.valueOf(params.get("highest"))<Box2d.scores)
		{
			max=Box2d .scores;
			label2=BitmapFontLabel.make(font2," highest score "+Box2d.scores);
			save();
		}else {
			label2=BitmapFontLabel.make(font2," highest score "+params.get("highest"));
		}
	...
	public void me()
	{
		...
		tex.autoRelease();
		tex2.autoRelease();
		MenuItemSprite shangchuan= MenuItemSprite.make(spriteNormal,spriteDisable,spriteDisable,this,"shangchuan");
		...
		Menu menu = Menu.make(playagain,shangchuan,menuItemSprite);
